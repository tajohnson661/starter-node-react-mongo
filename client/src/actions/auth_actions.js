...
import { AUTH_USER, AUTH_ERROR, UNAUTH_USER, AUTH_CLEAR_ERROR, ROOT_API_URL } from './constants';
  ...
  return function (dispatch) {
    ...
    axios.post(`${ROOT_AUTH_URL}/signin`, { email, password }) // { email: email, password: password }
      ...
      .catch((err) => {
        // If request is bad...
        // - show error to user
        console.log('response on signin:', err);
...
export const signUpUser = ({ email, password }) => {
  return function (dispatch) {
    axios.post(`${ROOT_AUTH_URL}/signup`, { email, password })
      ...
      .catch((err) => {
        console.log('err on signupuser:', err);
        dispatch(authError('Couldn\'t sign up user'));
      });
  };
};
